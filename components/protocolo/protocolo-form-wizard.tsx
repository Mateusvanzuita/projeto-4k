// src/components/protocolo/protocolo-form-wizard.tsx
"use client"
...
interface ProtocoloFormWizardProps {
  initialData?: any;
  isEditing?: boolean;
}

export function ProtocoloFormWizard({ initialData, isEditing = false }: ProtocoloFormWizardProps) {
  ...
  const [formData, setFormData] = useState(initialData || {
    ...
    manipulados: [],
    hormonios: [],
    treino: { cardio: { tipo: "", frequencia: "", tempo: "", observacoes: "" }, mobilidade: { quantidade: "", tipos: "" }, estrutura: "", divisoes: [] }
  })
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (isEditing && initialData?.id) {
        await protocoloService.update(initialData.id, payload as any);
      ...
      toast({ variant: "destructive", title: "Erro ao salvar", description: "Verifique os dados e tente novamente." });
    } finally {
        ...
        <div className="flex justify-between items-center bg-white p-4 border rounded-xl shadow-sm">
          ...
          <div className="flex gap-2">
            {currentStep < TOTAL_STEPS ? (
              ...
              <Button onClick={handleFinalize} disabled={isSubmitting} className="bg-green-600 hover:bg-green-700 px-8 text-white min-w-[180px]">
                {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Salvando...</> : <><Check className="mr-2 h-4 w-4" /> {isEditing ? "Salvar Alterações" : "Finalizar e Ativar"}</>}
              </Button>
            )}
          </div>
        </div>
