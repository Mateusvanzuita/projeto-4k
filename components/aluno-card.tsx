...
import { PLANO_LABELS, TIPO_PLANO_LABELS } from "@/types/aluno"
...
const OBJETIVO_COLORS: Record<string, string> = {
  "Perda de Peso": "bg-red-500/10 text-red-700 border-red-200",
  Hipertrofia: "bg-blue-500/10 text-blue-700 border-blue-200",
  Definição: "bg-purple-500/10 text-purple-700 border-purple-200",
  Saúde: "bg-green-500/10 text-green-700 border-green-200",
  Performance: "bg-orange-500/10 text-orange-700 border-orange-200",
  Reabilitação: "bg-yellow-500/10 text-yellow-700 border-yellow-200",
}
...
const TIPO_PLANO_COLORS: Record<string, string> = {
  ...
  FULL: "bg-purple-500/10 text-purple-700 border-purple-200",
}

export function AlunoCard({ aluno, onEdit, onDelete, onClick }: AlunoCardProps) {
  const imc = (aluno.peso / aluno.altura ** 2).toFixed(1)
    ...
    <Card className="p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => onClick(aluno.id)}>
      ...
      <div className="space-y-2 mb-3">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            {aluno.altura.toFixed(2)}m • {aluno.peso}kg • IMC: {imc}
          </span>
        </div>
      </div>

      <div className="flex flex-wrap gap-2">
        <Badge className={OBJETIVO_COLORS[aluno.objetivo] || "bg-gray-500/10 text-gray-700 border-gray-200"}>
          {aluno.objetivo}
        </Badge>
